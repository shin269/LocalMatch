...
import catsImage from '../images/culture.png'; // 이미지 파일 import

function Question3({ onAnswerChange }) {
  ...
  const handleNext = () => {
    if (selectedValue) {
      navigate('/question_4'); // 다음 페이지로 이동
    } else {
      alert('값을 선택해주세요!');
    }
  };
  ...
  const handlePrevious = () => {
    navigate('/question_2'); // 이전 페이지로 이동
  };

  return (
    <div
      style={{
        backgroundColor: '#FFB6C1', // 밝은 핑크색 배경
        minHeight: '100vh',
        display: 'flex',
    ...
    >
      ...
      >
        ...
        <div
          style={{
            flex: 1,
            backgroundColor: '#FFD1DC', // 연한 핑크색 배경
            padding: '20px',
            display: 'flex',
          ...
          {['매우 중요함', '중요함', '보통', '중요하지 않음', '매우 중요하지 않음'].map(
            (label, index) => (
              <label
                ...
                onMouseEnter={(e) => {
                  e.target.style.backgroundColor = '#ffc5d2'; // 호버 시 밝은 핑크 배경
                  e.target.style.boxShadow = '0 4px 10px rgba(0, 0, 0, 0.1)'; // 그림자 효과
                }}
                onMouseLeave={(e) => {
                  e.target.style.backgroundColor = '#ffffff'; // 기본 배경 복원
      ...
      <button
        ...
        onMouseLeave={(e) => {
          e.target.style.backgroundColor = '#ffcc00'; // 기본 배경 복원
      ...
      <button
        ...
        onMouseLeave={(e) => {
          e.target.style.backgroundColor = '#32CD32'; // 기본 초록색 복원
          e.target.style.transform = 'scale(1)';
        }}
      >
        다음
      ...
      <button
        ...
        onMouseEnter={(e) => {
          e.target.style.backgroundColor = '#C71585'; // 호버 시 더 어두운 핑크색
          e.target.style.transform = 'scale(1.1)'; // 살짝 확대
        }}
        onMouseLeave={(e) => {
          e.target.style.backgroundColor = '#FF1493'; // 기본 색상 복원
          e.target.style.transform = 'scale(1)';
        }}
        onMouseDown={(e) => {
          e.target.style.transform = 'scale(0.95)'; // 클릭 시 살짝 작아짐
