...
function Question4({ onAnswerChange }) {
  ...
  const handleNext = () => {
    if (selectedValue) {
      navigate('/question_5'); // 다음 페이지로 이동
    } else {
      alert('값을 선택해주세요!');
    }
  };
  ...
  const handlePrevious = () => {
    navigate('/question_3'); // 이전 페이지로 이동
  };

  return (
    <div
      style={{
        backgroundColor: '#ccffcc', // 밝은 연두색 배경
        minHeight: '100vh',
        display: 'flex',
    ...
    >
      ...
      >
        ...
        <div
          style={{
            flex: 1,
            backgroundColor: '#e6ffe6', // 연한 연두색 배경
            padding: '20px',
            display: 'flex',
          ...
          {['매우 중요함', '중요함', '보통', '중요하지 않음', '매우 중요하지 않음'].map(
            (label, index) => (
              <label
                ...
                onMouseEnter={(e) => {
                  e.target.style.backgroundColor = '#d4f5d4'; // 호버 시 연한 초록색 배경
      ...
      <button
        ...
        onMouseEnter={(e) => {
          e.target.style.backgroundColor = '#004d00'; // 호버 시 더 어두운 초록색
          e.target.style.transform = 'scale(1.1)'; // 살짝 확대
        }}
        onMouseLeave={(e) => {
          e.target.style.backgroundColor = '#006400'; // 기본 초록색 복원
          e.target.style.transform = 'scale(1)';
        }}
        onMouseDown={(e) => {
          e.target.style.transform = 'scale(0.95)'; // 클릭 시 살짝 작아짐
